...
import castBoolean from '../utils/cast-boolean.js';
...
class MembersHandlers {
    ...
    static async getMembers(ctx) {
        try {
            ...
            if (ctx.request.querystring) {
                const filter = Object.keys(ctx.request.query)
                    .map(function(q) {
                        return q + ' = :' + q;
                    })
                    .join(' and ');
            ...
            for (let m = 0; m < members.length; m++) {
                members[m] = {
                    _id:  members[m].MemberId,
                    _uri: '/members/' + members[m].MemberId,
                };
            }
   ...
   */
    static async getMemberById(ctx) {
        const result = await Db.query('Select * From Member Where MemberId = :id', {
            id: ctx.params.id,
        });
        const [ members ] = castBoolean.fromMysql(result);
        ...
        // team membership
        const sql =
      'Select TeamId As _id, concat("/teams/",TeamId) As _uri From TeamMember Where MemberId = :id';
